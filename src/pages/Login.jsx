    ...
    try {
      if (isRegistering) {
        if (!namaGuru.trim() || !username.trim() || !email.trim() || !password.trim()) {
          throw new Error('Nama Lengkap, Username, Email Resmi, dan Password wajib diisi.');
        }
        ...
        if (res.requiresEmailVerification) {
          setSuccessInfo(`📧 Link verifikasi telah dikirim ke ${res.email}. Silakan cek inbox/spam email Anda untuk mengonfirmasi akun sebelum login.`);
        ...
        if (!username.trim() || !password.trim()) {
          throw new Error('Masukkan Username/Email Resmi dan Password Anda.');
        }
